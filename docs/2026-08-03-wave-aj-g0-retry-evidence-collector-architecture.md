...
A future local composition layer owns protected challenge files, fixed policy bytes/digest, strict UTF-8 JSON parsing, lexical duplicate-key/depth rejection, owner-only file checks, an explicit production clock, replay handling, and fail-safe publication. It independently protects challenge ID, run ID, nonce, collector key ID, challenge digest, and the fixed policy digest. AJ-1 accepts those protected values but cannot establish their file provenance.

### Layer 2 — provider observation adapters (AJ-2)

Vercel, Railway, and Supabase adapters perform read-only control-plane observation and emit one provider-specific sanitized envelope each. Raw provider payloads, stdout/stderr, credentials, cookies, tokens, URLs, and authorization headers never cross the envelope boundary. Adapters do not create, update, delete, purge, deploy, or otherwise mutate provider state. Adapter transport, credential custody, executable pinning/containment, output limits, and duplicate-key-safe parsing are outside AJ-1.
...
The core fixes the target SHA, prior-created IDs/names, preview IDs, account/workspace identities, plans, USD currency, and strict all-in cap to the Wave AG values. Every prior-created resource must be absent by exact ID and exact name, with no pending deletion or tombstone. Every preview identity must be exact and explicitly unchanged.
...
### Layer 4 — signing, publication, and offline decision (AJ-3)

A later local layer may derive and sign the exact evidence and provider receipt with a separately protected Ed25519 key, atomically publish private artifacts, consume replay only at commit, and invoke the existing G0 retry eligibility validator. It must recompute challenge/evidence/inventory digests, verify the independently protected collector key, and retain the validator's no-hosted-authority/fresh-approval invariants. AJ-3 must not combine evidence collection with a provider mutation.
...
1. **AJ-1 (this change):** pure schemas, protected binding validation, decimal normalization, exact unsigned evidence/inventory composition, blocked conclusions, hostile tests, and this architecture record.
2. **AJ-2:** hardened local adapters and strict-byte/duplicate-key-safe intake under explicit read-only credentials and process/I/O containment. No signing or authority broadening merely because collection succeeds.
3. **AJ-3:** protected Ed25519 signing, digest chain, replay-safe owner-only publication, and in-memory/offline execution of the existing Wave AG validator. A successful validator result still only permits requesting a fresh approval.
...
```text
node --test scripts/g0-retry-evidence-collector-core.test.mjs
