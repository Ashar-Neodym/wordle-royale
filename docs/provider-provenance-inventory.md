...
Every CLI path must be absolute. Every input and signing file must be a no-follow, owner-owned regular file with mode `0600`. Output, executor-staging, and replay directories are owner-owned `0700`; committed JSON and replay markers are `0600`. Publication keeps the output root open with `O_DIRECTORY|O_NOFOLLOW` and uses its descriptor anchor even if its pathname is replaced. It creates the flat canonical names `RUN_ID.challenge.json`, `RUN_ID.evidence.json`, `RUN_ID.inventory.json`, and `RUN_ID.receipt.json` using `O_CREAT|O_EXCL|O_NOFOLLOW`, records each opened inode, writes and fsyncs it, then publishes `RUN_ID.commit.json` last. The canonical commit binds the exact component names and byte digests. The root is fsynced after components and commit. Verification receives the output root plus run ID, requires the canonical commit and exact run-prefixed set, and rejects partial, extra, replaced, or digest-mismatched bundles. Replay consumption remains descriptor-anchored and atomic with `O_EXCL`.

Publication never unlinks a run-prefixed evidence or commit path. On any failure it leaves every exclusive file and every raced replacement in place as fail-closed forensic evidence; without a valid canonical commit, verification rejects the run. A retry requires a fresh challenge and run ID rather than reuse or automatic cleanup. Retention is the default. Any later forensic cleanup is a separately reviewed and explicitly approved operator action; this collector provides no cleanup hook or generic janitor. The CLI may remove only its private transient executor staging directory, which is created under the system temporary directory outside the protected output root and contains no run evidence.
