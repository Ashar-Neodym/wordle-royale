...
4. **G3 — phased read-only preflight approval.** Supply Ticket 262's strict `wordle-provider-inventory/v2`, structured `wordle-provider-receipt/v2`, complete signed native evidence, out-of-band expected identities and nonce, and protected HMAC key file alongside the separate schema-v3 operational phase inventory. Run `pnpm --filter @wordle-royale/api auth:activation:preflight --operational-inventory <phase.json> --provider-inventory <provider.json> --provider-receipt <receipt.json> --native-evidence <native.json> --expected-identities <identities.json> --expected-nonce <run-id> --provider-receipt-key <key-file>` with the direct `DATABASE_URL` supplied only in the process environment. The CLI authenticates native signatures, HMAC receipt, freshness, nonce, and exact identities before loading database code or mapping any field. The operational run/source/artifact, Railway project/environment/service/deployment, Vercel project/deployment, and production/preview database identities must match authenticated provenance exactly. The full sanitized provider inventory (including every artifact, manifest/attestation, PostgreSQL replica observation, and provenance digest) plus structured receipt is bound into the canonical preflight evidence and receipt; only then can `providerDerived=true` be emitted. Caller claims, plain hashes, missing native evidence, stale/mixed evidence, and schema-v2 supplied as schema-v3 fail closed. The phase inventory must name exactly one `activationPhase`: `dormant`, `closed`, or `canary`; its run ID equals the expected provider nonce. Dormant requires durable API behavior disabled, registration closed, durable-auth readiness `not_checked_stub`, and the production web account mode disabled. Closed requires durable API readiness and web identity in closed mode. Canary requires durable API readiness and web identity in canary mode. The command permits only bounded (64 KiB), five-second-timeout public GETs with manual redirects, requires exact JSON content types/schemas, and independently probes the API and `/.well-known/wordle-identity` web revision. It establishes the baseline and strict database checks in a repeatable-read `READ ONLY` transaction, then opens a separate post-probe `READ ONLY` transaction to observe zero writes visible after the GETs. Both transactions read back `SHOW transaction_read_only`. The command checks the complete migration set and direct-host fingerprint, and requires PostgreSQL permission to execute `pg_control_system()`; missing permission is the hard blocker `pg_control_system_execute_required`. It rejects `--apply` and unknown flags. Capture canonical stdout and its receipt; do not retry by weakening scope.
