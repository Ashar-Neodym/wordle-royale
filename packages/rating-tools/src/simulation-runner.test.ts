...
test('comparison reports at least three parameter sets across required scenarios', () => {
  ...
  assert.equal(report.parameterSets.length >= 4, true);
  assert.equal(report.modeLadders.length, 4);
  assert.equal(report.modeLadders.some((ladder) => ladder.mode === 'standard_1v1'), true);
