...
} from '../lib/server-action-presentation';
...
export async function startPreviewDemoSessionAction(formData: FormData): Promise<void> {
  return runPreviewDemoServerAction(async () => {
  ...
  }, () => redirect(PREVIEW_DEMO_ACTION_UNAVAILABLE_PATH));
}

export async function createStandard1v1TicketAction(): Promise<ApiClientResult<Standard1v1Ticket>> {
  return runApiClientServerAction(() => {
    const body: CreateStandard1v1TicketRequest = {
      clientRequestId: randomUUID(), mode: 'standard_1v1', rated: true, allowProvisionalOpponent: true,
    };
...
export async function markSpeedMatchReadyAction(matchId: string, clientRequestId: string): Promise<ApiClientResult<SpeedMatchSnapshot>> {
  return runApiClientServerAction(() => runSpeedServerAction(() => markSpeedMatchReady(matchId, { clientRequestId })));
}
...
export async function forfeitSpeedMatchAction(matchId: string, clientRequestId: string): Promise<ApiClientResult<SpeedMatchSnapshot>> {
  return runApiClientServerAction(() => runSpeedServerAction(() => forfeitSpeedMatch(matchId, { clientRequestId })));
}
...
export async function submitSpeedGuessAction(matchId: string, roundId: string, guess: string, clientRequestId: string): Promise<ApiClientResult<GuessResult>> {
  return runApiClientServerAction(() => runSpeedServerAction(() => submitSpeedGuess({ clientRequestId, matchId, roundId, guess: guess.toLowerCase(), clientSubmittedAt: new Date().toISOString() })));
}

export async function createRankedLobbyAction(): Promise<void> {
  return runOperationalServerAction(async () => {
    const result = await createLobby({ ...rankedLobbyDefaults, clientRequestId: randomUUID() });
    if (result.status === 'connected' && result.data) resultRedirect({ action: 'create_lobby', status: 'success', lobbyId: result.data.id, lobbyCode: result.data.code });
...
export async function joinLobbyByCodeAction(formData: FormData): Promise<void> {
  return runOperationalServerAction(async () => {
    ...
    if (!code) resultRedirect({ action: 'join_code', status: 'error', message: 'Enter a lobby code first.' });
    const result = await joinLobbyByCode({ clientRequestId: randomUUID(), code });
    if (result.status === 'connected' && result.data) resultRedirect({ action: 'join_code', status: 'success', lobbyId: result.data.id, lobbyCode: result.data.code });
...
export async function joinLobbyAction(formData: FormData): Promise<void> {
  return runOperationalServerAction(async () => {
    ...
    if (!lobbyId) resultRedirect({ action: 'join_lobby', status: 'error', message: 'Missing lobby id.' });
    const result = await joinLobby(lobbyId, randomUUID());
    if (result.status === 'connected' && result.data) resultRedirect({ action: 'join_lobby', status: 'success', lobbyId: result.data.id, lobbyCode: result.data.code });
...
export async function startRankedMatchAction(formData: FormData): Promise<void> {
  return runOperationalServerAction(async () => {
    const lobbyId = formValue(formData, 'lobbyId');
    if (!lobbyId) resultRedirect({ action: 'start_ranked', status: 'error', message: 'Missing lobby id.' });
    const result = await startRankedMatch({ clientRequestId: randomUUID(), lobbyId, source: 'lobby' });
    if (result.status === 'connected' && result.data) resultRedirect({ action: 'start_ranked', status: 'success', lobbyId, matchId: result.data.matchId, roundId: result.data.roundId });
...
export async function submitRankedGuessAction(formData: FormData): Promise<void> {
  return runOperationalServerAction(async () => {
    ...
    if (!matchId || !roundId) resultRedirect({ action: 'submit_guess', status: 'error', message: 'Missing match or round id.' }, 'gameplay');
    if (!/^[a-z]{5}$/.test(guess)) resultRedirect({ action: 'submit_guess', status: 'error', matchId, roundId, message: 'Enter exactly five letters.' }, 'gameplay');
    ...
    if (result.status === 'connected' && result.data) {
      ...
      resultRedirect({ action: 'submit_guess', status: 'success', matchId, roundId, guessStatus, message: result.data.accepted ? `Guess ${guess.toUpperCase()} accepted.` : `Guess ${guess.toUpperCase()} rejected: ${result.data.reason}.` }, 'gameplay');
    }
...
export async function completeRankedMatchAction(formData: FormData): Promise<void> {
  return runOperationalServerAction(async () => {
    const matchId = formValue(formData, 'matchId');
    if (!matchId) resultRedirect({ action: 'complete_match', status: 'error', message: 'Missing match id.' }, 'gameplay');
