...
const json=(response,body)=>{const bytes=Buffer.from(JSON.stringify(body));response.writeHead(200,{'content-type':'application/json','content-length':String(bytes.length)});response.end(bytes);};
const spawnCli=(args,env={})=>new Promise((resolve,reject)=>{const child=spawn('pnpm',['--filter','@wordle-royale/api','auth:activation:preflight',...args],{cwd:workspaceRoot,env:{...process.env,DATABASE_URL:database.toString(),NODE_ENV:'test',RUN_AUTH_PREFLIGHT_CLI_E2E:'1',...env},stdio:['ignore','pipe','pipe']});let stdout='',stderr='';child.stdout.setEncoding('utf8').on('data',chunk=>stdout+=chunk);child.stderr.setEncoding('utf8').on('data',chunk=>stderr+=chunk);child.once('error',reject);child.once('close',status=>resolve({status,stdout,stderr}));});
...
try {
  ...
  receiptKey.fill(0);
  const argsFor=(overrides={},extra=[])=>['--operational-inventory',overrides.inventory??files.inventory,'--provider-inventory',overrides.provider??files.provider,'--provider-receipt',overrides.receipt??files.receipt,'--native-evidence',overrides.native??files.native,'--expected-identities',overrides.identities??files.identities,'--expected-nonce',overrides.nonce??nonce,'--provider-receipt-key',overrides.key??files.key,...extra];

  let publicRequests=0,mutateOnRequest=false,mutationDone=false;
  const apiLocal=await listen(async(request,response)=>{publicRequests++;if(mutateOnRequest&&!mutationDone){mutationDone=true;sync('psql',[admin.toString(),'-X','-v','ON_ERROR_STOP=1','-c',`INSERT INTO "${schema}"."AuditLog" (id,action,"entityType") VALUES ('ticket268-drift','public_probe','fixture')`]);}const path=new URL(request.url,'http://local').pathname;if(path==='/healthz')return json(response,envelope({status:'ok',service:'wordle-royale-api',environment:'production',timestamp:new Date().toISOString(),uptimeSeconds:1,revision:operationalInventory.deployments.apiRevision}));if(path==='/readyz')return json(response,envelope({status:'ok',service:'wordle-royale-api',environment:'production',revision:operationalInventory.deployments.apiRevision,checkedAt:new Date().toISOString(),dependencies:{database:dependency(),applicationSchema:dependency(),durableAuth:{...dependency(),registrationMode:'canary',keyFingerprint:operationalInventory.config.keyFingerprint,configFingerprint:operationalInventory.config.configFingerprint,expectedReplicaCount:1},standardDictionary:dependency(),speedRuntime:dependency(),speedLifecycleActivation:dependency(),redis:dependency()}}));if(path==='/ranked/modes')return json(response,envelope(modes()));response.writeHead(404).end();});
  const previewLocal=await listen((request,response)=>{publicRequests++;json(response,envelope({status:'ok',service:'wordle-royale-api',environment:'production',revision:preview.railway.artifact.sourceGitSha,checkedAt:new Date().toISOString(),dependencies:{database:dependency(),applicationSchema:dependency(),durableAuth:{...dependency('not_checked_stub'),registrationMode:'closed'},standardDictionary:dependency(),speedRuntime:dependency(),speedLifecycleActivation:dependency(),redis:dependency()}}));});
  const webLocal=await listen((request,response)=>{publicRequests++;json(response,{revision:operationalInventory.deployments.webRevision,appEnvironment:'production',mode:'durable',registrationMode:'canary'});});
