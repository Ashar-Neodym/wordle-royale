...
const baseDatabaseUrl = process.env.SPEED_RACE_TEST_DATABASE_URL
  ?? process.env.DATABASE_URL
  ?? `postgresql://wordle:${localPassword}@localhost:5432/wordle_royale_local?schema=public`;
const iterations = Number.parseInt(process.env.SPEED_RACE_ITERATIONS ?? '10', 10);
if (!Number.isInteger(iterations) || iterations < 1 || iterations > 25) throw new Error('SPEED_RACE_ITERATIONS must be an integer from 1 through 25.');

const databaseUrlForSchema = (schema) => {
  const url = new URL(baseDatabaseUrl);
  url.searchParams.set('schema', schema);
  url.searchParams.set('application_name', `ticket185_${schema}`.slice(0, 63));
  return url.toString();
};
