...
const schemaName = `ticket158_${process.pid}_${Date.now()}`;
...
let exitCode = 0;
console.log(`Ticket 158 PostgreSQL integration schema: ${schemaName}`);
try {
  await admin.$executeRawUnsafe(`CREATE SCHEMA "${schemaName}"`);
  run('pnpm', ['db:migrate:deploy'], { DATABASE_URL: schemaDatabaseUrl });
  run('pnpm', ['db:seed:local'], { DATABASE_URL: schemaDatabaseUrl });
  run('pnpm', ['exec', 'node', '--import', 'tsx', '--test', 'test/speed-gameplay-postgres.integration.test.ts'], {
    APP_ENV: 'test', AUTH_MODE: 'dev_stub', DATABASE_URL: schemaDatabaseUrl,
    ENABLE_DEV_AUTH: 'true', ENABLE_DEV_ROUTES: 'true', NODE_ENV: 'test',
    RUN_SPEED_POSTGRES_INTEGRATION: '1', STANDARD_1V1_QUEUE_ENABLED: 'true', SPEED_1V1_QUEUE_ENABLED: 'true',
  });
} catch (error) {
  exitCode = 1;
  ...
  try {
    ...
    console.log(`Dropped Ticket 158 PostgreSQL integration schema: ${schemaName}`);
  } catch (error) {
    exitCode = 1;
    console.error(error instanceof Error ? error.message : error);
  }
