  ...
  try {
    ...
    for (const env of ENVS) {
      ...
      for (const [index, raw] of pg.observations.entries()) {
        ...
        const d = digest(raw.evidenceDigest, `${p}.evidenceDigest`); if (evidenceDigests.has(d)) fail('DUPLICATE_EVIDENCE_DIGEST', p); evidenceDigests.add(d); timestamp(raw.observedAt, `${p}.observedAt`); if (raw.physicalNodeId !== null) environmentResources[env].add(id(raw.physicalNodeId, `${p}.physicalNodeId`));
