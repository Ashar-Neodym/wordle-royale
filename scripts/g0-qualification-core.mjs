...
export function validateManifest(m) {
  ...
  const zeroes = [m.productionShells.vercel.deployments,m.productionShells.vercel.environmentVariables,m.productionShells.vercel.domains,m.productionShells.railway.apiServingReplicas,m.productionShells.railway.postgresServiceCount,m.productionShells.railway.deployments,m.productionShells.railway.variables,m.productionShells.railway.domains,m.productionShells.supabase.newProjects,m.productionShells.redis.services];
  fail(zeroes.every((value) => value === '0') && m.productionShells.vercel.sourceLinkage === 'forbidden' && m.productionShells.railway.sourceLinkage === 'forbidden' && m.productionShells.railway.postgresDeferredGate === 'G2', 'ZERO_ACTION_POLICY_INVALID');
  fail(m.productionShells.vercel.projectName === 'wordle-royale-production-web' && m.productionShells.railway.projectName === 'wordle-royale-production' && m.productionShells.vercel.projectName !== m.previewPreservation.vercel.projectName && m.productionShells.railway.projectName !== m.previewPreservation.railway.projectName, 'PREVIEW_PRODUCTION_OVERLAP');
  exactKeys(m.plannedTopology, ['railwayRegion','apiReplicasAtDormantDeploy','apiAndPostgresPrivateNetwork','vercelRuntimeRegionPreference','plannedWebOrigin','plannedApiOrigin'], 'TOPOLOGY_SCHEMA_INVALID'); fail(m.plannedTopology.plannedWebOrigin === null && m.plannedTopology.plannedApiOrigin === null, 'PUBLIC_ORIGIN_NOT_NULL');
  ...
  fail(m.cost.currency === 'USD' && m.cost.railwayTargetPlan === 'Hobby' && usage <= cap && minimum === 50000n && cap === 50000n && decimalParts(m.cost.vercelApprovedMonthlyCharge, 'COST_INVALID') === 0n && decimalParts(m.cost.supabaseApprovedMonthlyCharge, 'COST_INVALID') === 0n && m.cost.overageApproved === false && m.cost.paidAddonsApproved === false && m.cost.planUpgradeUnderThisManifest === false, 'COST_POLICY_INVALID');
  ...
  fail(Array.isArray(m.forbidden) && m.forbidden.length === 10 && m.forbidden.includes('preview mutation or reuse') && m.forbidden.includes('git repository linkage') && m.forbidden.includes('paid upgrade, overage, add-on, invitation, or payment-method change'), 'FORBIDDEN_POLICY_INVALID');
  exactKeys(m.rollback, ['authorizedOnlyForNewUnambiguousIds','triggers','order'], 'ROLLBACK_SCHEMA_INVALID'); fail(m.rollback.authorizedOnlyForNewUnambiguousIds === true && m.rollback.triggers.length === 4 && m.rollback.order.length === 5, 'ROLLBACK_POLICY_INVALID');
  fail(Array.isArray(m.blockingPrerequisites) && m.blockingPrerequisites.length === 4, 'BLOCKER_SCHEMA_INVALID');
  const expected = { LIVE_PROVIDER_PROVENANCE:'resolved_by_wave_ad', SINGLE_NODE_POSTGRES_OBSERVATION:'resolved_by_wave_ad', RAILWAY_BACKUP_RESTORE:'unresolved_before_G2', PLAN_AND_COST:'approval_required' };
  for (const blocker of m.blockingPrerequisites) { exactKeys(blocker, ['id','state','reason'], 'BLOCKER_SCHEMA_INVALID'); fail(Object.hasOwn(expected, blocker.id) && blocker.state === expected[blocker.id] && typeof blocker.reason === 'string' && blocker.reason.length >= 20, 'BLOCKER_CLASSIFICATION_INVALID'); delete expected[blocker.id]; }
