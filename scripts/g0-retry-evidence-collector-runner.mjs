...
const SUPERVISOR_SHA='sha256:c4e9373b5fe5f118f1673e776188a4554634c656b1aac4a2d497a797817ea00c';
const PYTHON='/usr/bin/python3', MAX_FRAME=1_048_576;
...
export function createG0RetryAdapterRunner({totalTimeoutMs=30_000}={}){
 if(!Number.isInteger(totalTimeoutMs)||totalTimeoutMs<300||totalTimeoutMs>900_000)fail('INVALID_TOTAL_TIME_LIMIT');const begun=Date.now(),seen=new Set();let closed=false;
 return{async run({provider,executable,argv,limits}){if(closed||!['vercel','railway','supabase'].includes(provider)||seen.has(provider))fail('INVALID_RUNNER_SPEC');seen.add(provider);policyFields(executable);if(!Array.isArray(argv)||argv.some(x=>typeof x!=='string'||x.includes('\0')))fail('INVALID_RUNNER_SPEC');for(const x of ['timeoutMs','stdoutBytes','stderrBytes'])if(!Number.isInteger(limits?.[x])||limits[x]<1||limits[x]>1_048_576)fail('INVALID_RUNNER_LIMITS');
  const opened=await inspect(executable.path);let supervisor;try{if(opened.realpath!==executable.realpath||opened.snapshot.uid!==process.getuid?.()||opened.snapshot.mode!==0o500||opened.snapshot.nlink!==1||opened.snapshot.sha256!==executable.sha256)fail('EXECUTABLE_POLICY_MISMATCH');supervisor=await start(opened.handle.fd);const remaining=totalTimeoutMs-(Date.now()-begun),timeoutMs=Math.min(remaining,limits.timeoutMs);if(timeoutMs<1)fail('TOTAL_TIME_LIMIT');const response=await supervisor.request({type:'run',seq:1,argv,timeoutMs,stdoutBytes:limits.stdoutBytes,stderrBytes:limits.stderrBytes});if(!response||response.type!=='result'||response.seq!==1||typeof response.ok!=='boolean')fail('CONTAINMENT_HELPER_FAILED');if(!response.ok){if(!['PROCESS_TIMEOUT','STDOUT_LIMIT','STDERR_LIMIT','PROCESS_SPAWN_FAILED','DESCENDANT_CLEANUP_FAILED'].includes(response.code))fail('CONTAINMENT_HELPER_FAILED');fail(response.code);}if(Object.keys(response).sort().join('|')!=='exitCode|ok|seq|signal|stderr|stdout|type'||!Number.isInteger(response.exitCode)||response.signal!==null)fail('CONTAINMENT_HELPER_FAILED');const stdout=strictBase64(response.stdout,limits.stdoutBytes),stderr=strictBase64(response.stderr,limits.stderrBytes);if(response.exitCode!==0)fail('ADAPTER_NONZERO_EXIT');if(stderr.length)fail('ADAPTER_STDERR_FORBIDDEN');const after=await inspect(executable.path);try{if(after.realpath!==opened.realpath||!snapshotSame(after.snapshot,opened.snapshot))fail('EXECUTABLE_CHANGED');}finally{await after.handle.close();}if(!await supervisor.close(2))fail('CONTAINMENT_HELPER_FAILED');supervisor=undefined;return stdout;
  }finally{if(supervisor)await supervisor.close(2).catch(()=>{});await opened.handle.close();}},async finish(){if(seen.size!==3)fail('INCOMPLETE_ADAPTER_EXECUTION');closed=true;},async close(){closed=true;}};
}
