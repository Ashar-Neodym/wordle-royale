...
function parseEvidence(raw, c, policy) {
  ...
  exact(raw.cleanup, ['destinationIdentity','checkedAt','resourceLookup','pendingDeletion','tombstone'], 'evidence.cleanup'); sameIdentity(identity(raw.cleanup.destinationIdentity, 'cleanup.destinationIdentity'), policy.identities.restoreDestination, 'CLEANUP_IDENTITY_MISMATCH'); const cleanupChecked = eventTime(raw.cleanup.checkedAt, 'cleanup.checkedAt'); fail(cleanupChecked >= verificationCompleted, 'CLEANUP_TIME_ORDER_INVALID'); same(cleanupChecked, observedAt, 'CLEANUP_NOT_AGGREGATE_OBSERVATION'); same(raw.cleanup.resourceLookup, 'absent', 'CLEANUP_RESOURCE_PRESENT'); same(raw.cleanup.pendingDeletion, false, 'CLEANUP_PENDING'); same(raw.cleanup.tombstone, false, 'CLEANUP_TOMBSTONE_REMAINS');
  exact(raw.productionNoMutation, ['identity','windowStartedAt','windowCompletedAt','mutationCount','confirmed'], 'evidence.productionNoMutation'); sameIdentity(identity(raw.productionNoMutation.identity, 'productionNoMutation.identity'), policy.identities.sourceProduction, 'PRODUCTION_IDENTITY_MISMATCH'); const noMutationStart = eventTime(raw.productionNoMutation.windowStartedAt, 'productionNoMutation.windowStartedAt'); const noMutationComplete = eventTime(raw.productionNoMutation.windowCompletedAt, 'productionNoMutation.windowCompletedAt'); fail(noMutationStart <= recoveryPoint && noMutationStart <= backupStart && noMutationComplete >= cleanupChecked && noMutationComplete === observedAt, 'PRODUCTION_WINDOW_INADEQUATE'); same(raw.productionNoMutation.mutationCount, '0', 'PRODUCTION_MUTATION_OBSERVED'); same(raw.productionNoMutation.confirmed, true, 'PRODUCTION_NO_MUTATION_UNPROVEN');
...
export function evaluateG2BackupRestoreReadiness({ challenge:challengeRaw, evidence, providerReceipt:receipt, collectorPublicKey, policy:policyRaw, now }) {
  ...
  const evaluationNow = now instanceof Date ? now.getTime() : now;
  fail(Number.isFinite(evaluationNow), 'INVALID_FRESHNESS_POLICY');
  fail(Date.parse(evidence.observedAt) <= evaluationNow, 'FUTURE_EVIDENCE');
