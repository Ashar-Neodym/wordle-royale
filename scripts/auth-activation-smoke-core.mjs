...
function fingerprint(runId, email) { return createHash('sha256').update(`auth-smoke-account-v1\0${runId}\0${email.trim().toLowerCase()}`).digest('hex'); }
export function validateApproval(v, preflight, secrets, now = Date.now()) {
  ...
  fail(v.accountFingerprint === fingerprint(v.runId, secrets.email), 'approval_account_binding_mismatch');
...
export const SMOKE_RECONCILIATION_SQL = Object.freeze({
  ...
  snapshot: 'SELECT auth_activation_smoke_reconciliation_v1($1,$2)',
});
const RECONCILIATION_KEYS = ['accountCount','profileCount','credentialCount','sessionCount','terminalSessionCount','activeSessionCount','registerAttempts','loginAttempts','registerBucketCount','loginBucketCount','blockedBucketCount','ticketWriteCount','matchWriteCount','gameplayWriteCount','ratingWriteCount','eventWriteCount','catalogFingerprint'];
...
const TRANSPORT_RECORD_KEYS = ['method','url','effectiveUrl','origin','redirect','status','contentType','bodyBytes','setCookie','body','elapsedMs'];
...
async function request(transport, approval, method, path, options = {}) {
  ...
  const url = new URL(path, `${approval.origins.api}/`).href;
  const r = await transport.request({ method, url, redirect: 'manual', origin, ...options });
  ...
  fail(effective.protocol === 'https:' && effective.origin === approval.origins.api, 'transport_response_authority_invalid');
  noLeak(r); return r;
}
...
export async function runAuthActivationSmoke({ approval, preflight, secrets, transport, reconciliation, consumeApproval, now = () => Date.now() }) {
  const evidence = { schemaVersion: 1, result: 'FAIL', runId: typeof approval?.runId === 'string' ? approval.runId : 'invalid', approvalConsumed: false, registerDispatches: 0, statuses: [], sessionsCreated: 0, checks: { identity: false, negative: false, cookieAmbiguity: false, enumerationGeneric: false, registration: false, secureHostCookie: false, me: false, logoutReplay: false, reloginRevocation: false, finalZeroActive: false, rankedUnchanged: false } };
  let jar = null;
  try {
    ...
    fail(before.accountCount === 0 && before.profileCount === 0 && before.credentialCount === 0 && before.sessionCount === 0 && before.terminalSessionCount === 0 && before.activeSessionCount === 0, 'baseline_not_clean');
    fail(['registerAttempts','loginAttempts','registerBucketCount','loginBucketCount','blockedBucketCount','ticketWriteCount','matchWriteCount','gameplayWriteCount','ratingWriteCount','eventWriteCount'].every((key) => before[key] === 0) && typeof before.catalogFingerprint === 'string' && RECEIPT.test(before.catalogFingerprint), 'baseline_not_clean');
    let catalogBefore;
    ...
    await consumeApproval({ approvalId: approval.approvalId, runId: approval.runId, preflightReceipt: approval.preflightReceipt });
    ...
    fail(after?.accountCount === 1 && after?.profileCount === 1 && after?.credentialCount === 1 && after?.sessionCount === 3 && after?.terminalSessionCount === 3 && after?.activeSessionCount === 0, 'terminal_auth_cardinality_invalid');
    fail(after.ticketWriteCount === 0 && after.matchWriteCount === 0 && after.gameplayWriteCount === 0 && after.ratingWriteCount === 0 && after.eventWriteCount === 0 && after.catalogFingerprint === before.catalogFingerprint, 'ranked_mutation_detected');
    ...
    fail(evidence.statuses.length === 20 && evidence.statuses.join(',') === '200,200,200,403,201,200,204,401,403,403,403,401,401,200,200,401,200,204,401,200', 'status_cardinality_invalid');
    evidence.checks.finalZeroActive = evidence.checks.rankedUnchanged = true; evidence.result = 'PASS';
