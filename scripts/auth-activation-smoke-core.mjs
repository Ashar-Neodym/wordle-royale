...
const RECONCILIATION_KEYS = ['accountCount','profileCount','credentialCount','consentCount','sessionCount','terminalSessionCount','activeSessionCount','accountIdentityFingerprint','nonTargetSessionCount','nonTargetSessionFingerprint','nonTargetAuthCount','nonTargetAuthFingerprint','nonTargetRateLimitCount','nonTargetRateLimitFingerprint','registerAttempts','loginAttempts','registerBucketCount','loginBucketCount','blockedBucketCount','lobbyWriteCount','speedWriteCount','ticketWriteCount','matchWriteCount','gameplayWriteCount','mutationWriteCount','ratingWriteCount','eventWriteCount','sharedStateFingerprint','sharedStateUnchanged','catalogFingerprint'];
export function assertNonTargetRateLimitUnchanged(before, after) {
  fail(after.nonTargetRateLimitCount === before.nonTargetRateLimitCount && after.nonTargetRateLimitFingerprint === before.nonTargetRateLimitFingerprint, 'non_target_rate_limit_mutation_detected');
}
...
export async function runAuthActivationSmoke({ approval, preflight, secrets, transport, reconciliation, consumeApproval, now = () => Date.now() }) {
  ...
  try {
    ...
    assertNonTargetRateLimitUnchanged(before, after);
